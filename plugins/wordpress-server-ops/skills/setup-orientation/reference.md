...
2. Frage, ob die Plugin-Skills im Remote-SSH-Workspace verfügbar sind: `setup-orientation`, `wp-media-import`, `grill-me`, `wst-section-workflow`, `wst-new-post-type`, `frontend-section-qa`, `cpt-frontend-qa`. `setup-playwright-mcp` ist lokal-only und wird im lokalen Cursor-Workspace geprüft, nicht hier.
