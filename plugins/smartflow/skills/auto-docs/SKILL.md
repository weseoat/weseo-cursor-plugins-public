...
Legacy fallbacks — when the project has no `acf-json/` yet: legacy PHP field groups under `smart-template-builder/acf/field-groups/*.php` are documented from the PHP source the same way. When the project only maintains an admin JSON export, copy [scripts/generate.js](scripts/generate.js) to `docs/field-groups/generate.js` (when missing or older than the Skill copy) and run `node docs/field-groups/generate.js` from the repository root; it picks the newest `acf_export/acf-export-*.json`, writes one file per group, and generates its own `docs/field-groups/README.md` — do not overwrite that one in step 6. When no definition source exists, inform the user immediately and skip field-groups without aborting the other categories. When `acf-json/` is missing on a SmartFlow project, name the cause in the same breath — the ACF Local JSON gate (`setup-acf-local-json`, `setup-local-project` Step 14) has not run yet — write the field tables of the affected Section and CPT docs with `unbekannt` markers, put `field-groups: todo (rerun auto-docs after acf_local_json)` into the README index, and tell the user that one second `/auto-docs` run after the JSON setup fills the gap. Never document the missing folder as a final state.
