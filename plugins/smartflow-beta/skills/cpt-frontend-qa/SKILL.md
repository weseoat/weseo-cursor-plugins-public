...
When a CPT display becomes primarily a dedicated WST Section layout, route Section-level layout behavior to `frontend-section-qa`; card, archive/grid, carousel/filter, WPGB output, and optional single-template QA stay in this Skill. Under package orchestration this Skill runs through the `cpt-visual-implementer` runner per the `agent-routing` Rule; the semantics are identical either way.
