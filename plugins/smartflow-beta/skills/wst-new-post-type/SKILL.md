...
This Skill does not own CPT CSS or SCSS. It documents CSS paths, stable classes, hooks, and expected behavior in the work record; CSS implementation belongs to `cpt-frontend-qa` (Section-level CPT displays route Section behavior to `frontend-section-qa`). When work runs under the package orchestration, the foundation run executes through the `wst-shortcode-implementer` runner and WPGB work through the `wpgb-specialist`, per the `agent-routing` Rule.
