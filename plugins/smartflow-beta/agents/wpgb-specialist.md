...
- The bridge routes `wso/v1/wpgb/*` (read, update, create, reindex) are **the** standard write route when both gates hold: (a) the installed bridge is version 1.1.0 or newer (mandatory version comparison per the `status-bridge` Rule), and (b) `PROJECT-CONTEXT.md` records the WPGB bridge write route as project-validated. The gate stays — never flip a project to bridge writes silently.
...
4. **Order: card before grid.** Grids reference card IDs — create the card first and wire its id into the grid payload.
