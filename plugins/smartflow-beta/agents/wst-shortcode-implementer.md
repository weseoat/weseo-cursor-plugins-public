...
- Never: the WST plugin folder, `functions.php`, `theme-functions.php` (explicit user confirmation only), CSS/SCSS/`styles.json` (that scope belongs to `cpt-visual-implementer`; the Skill documents `CSS status: new-needed-for-frontend` plus target paths), the Section preview harness setup and its files (the `section-preview-harness` Skill runs in the main chat per the `agent-routing` Rule — surface the offer as `OPEN DECISION`), commits/pushes, cache or permalink flushes, files inside the deploy path that are not theme source.
